...
async function getBreakdown(days: number) {
  async function byField(field: string): Promise<BreakdownRow[]> {
    const res = await pool.query(
      `SELECT
         ...
         ROUND(AVG(cp.comments))::int AS avg_comments,
         ROUND(AVG(cp.shares))::int AS avg_shares,
  ...
  const [by_hook_type, by_content_type, by_layout, by_cta_keyword] = await Promise.all([
    byField('hook_type'),
    byField('content_type'),
    byField('layout'),
    byField('cta_keyword'),
  ])
  return { by_hook_type, by_content_type, by_layout, by_cta_keyword }
}
...
export async function extractPatterns(days: number): Promise<ViralPatterns> {
  ...
  function aggregate(field: 'hook_type' | 'cta_keyword' | 'content_type' | 'layout') {
    const map: Record<string, { total_views: number; count: number; reviewed_count: number; drove_sales_count: number }> = {}
    for (const row of postData) {
      const val = row[field]
      if (!val) continue
      if (!map[val]) map[val] = { total_views: 0, count: 0, reviewed_count: 0, drove_sales_count: 0 }
    ...
    const keyName = field === 'hook_type' ? 'style' : field === 'cta_keyword' ? 'keyword' : field === 'content_type' ? 'type' : 'layout'
    return Object.entries(map)
      .map(([key, v]) => ({
        ...
        drove_sales_rate: v.reviewed_count > 0
  ...
  const sampleWarning = baseline.post_count < 5
    ? `Only ${baseline.post_count} posts in the last ${days} days — patterns may not be reliable yet. Keep posting.`
    : null

  const postDataById = new Map(postData.map(p => [p.instagram_media_id, p]))
  const enrichedOutliers: OutlierPost[] = outliers.map(o => {
    const meta = postDataById.get(o.instagram_media_id)
    return {
      ...o,
      hook_type: meta?.hook_type ?? null,
      content_type: meta?.content_type ?? null,
      layout: meta?.layout ?? null,
      cta_keyword: meta?.cta_keyword ?? null,
    }
  })
